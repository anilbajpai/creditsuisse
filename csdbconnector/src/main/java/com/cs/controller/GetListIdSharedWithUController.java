...
@RequestMapping("/GetListIdSharedWithUController")

@Api(basePath = "/GetListIdSharedWithUController", value = "GetListIdSharedWithUController", description = "Operations with Db", produces = "application/json")
@RestController
public class GetListIdSharedWithUController {
	char ch = '"';
	...
	public @ResponseBody String getDetails(@RequestParam Long userId) {
		...
		try {
            ...
            String dataQryOuter ="select eul.userid,eul.listid,el.listdesc,eul.create_time,(select count(1) from journolistslinkage j where  j.listid = eul.listid and j.active='Y') contacts , eul.sharedbyuser,eul.owner from  emailuserlistlinkage eul,emaillists el where eul.listid = el.listid and  eul.userid='"+userId+"' and eul.sharedbyuser <> eul.userid and eul.active<>'N' and el.active <>'N' ";
