...
@Api(basePath = "/CreateInstrument", value = "CreateInstrument", description = "Operations with Db", produces = "application/json")
@RestController
public class CreateInstrumentController {
	...
	public @ResponseBody String saveChangesPost(
			@RequestBody(required = true) InstrumentPojo insertRequest) {
		InstrumentMst response = new InstrumentMst();
		response.setInstrumentDesc(insertRequest.getInstrumentDesc());
		response.setIsActive(insertRequest.getIsActive());
		SqlSession sqlSession = MyBatisSqlSessionFactory.getSqlSessionFactory().openSession();
		try {
			MybatisAbstractDao<InstrumentMst, Serializable, BaseEntityExample> dao = new MybatisAbstractDao<InstrumentMst, Serializable, BaseEntityExample>();
			dao.insert(response);
			sqlSession.commit();
			sqlSession.close();
			return "SUCCESS_POST";
